...
def create_team(bal, stat, name):
    ...
    if name == '':
        ...
        if bal == 'Balanced (One of Each Position)':
        ...
        elif bal == 'Unbalanced (Strictly Top Players)':
            #Determine which stat to use to filter
    ...
    #Print confirmation messages for user
    print('')
    print('Great! You are responsible for the team: ' + TEAM.team + '.')
    print('The players that had the best ' + stat + ' and who are on your team are: '\
          + str(TEAM.players.keys()))
...
def create_opponent(team):
    ...
    print('')
    print("The " + team + "'s players with the best " + CHOSEN_STAT + " are: ")
    print(str(OPPONENT.players.keys()))
...
def final_path(option):
    ...
    if option == 'Create New Team':
        ...
        print('')
        print('Okay, thanks for making your team! Bye!')
...
def build_team():
    ...
    button1 = tk.Button(options, text='Submit', command=lambda: \
                        create_team(variable1.get(), variable2.get(), name.get()))
    button1.pack()
    ...
    #Set dimensions for window
    nba_teams.geometry('400x600')
    #Create instructions and who will be chosen from the NBA team
    label5 = tk.Label(nba_teams, text='The top 5 players in chosen stat and team will be taken')
    label5.pack(pady=10)
    ...
    button2 = tk.Button(nba_teams, text='Submit', command=lambda:\
                        create_opponent(lstbx.get(lstbx.curselection())))
    button2.pack()
    #Create instructions to close TKinter window once finished
    ...
    final_lstbx = tk.Listbox(final, selectmode='SINGLE')
    ...
    button3 = tk.Button(final, text='Submit', command=lambda:\
                        final_path(final_lstbx.get(final_lstbx.curselection())))
    button3.pack()
    #Create instructions to close TKinter window once finished
    label6 = tk.Label(final, text='If Creating New Team press Enter to close this window and move on.\n Otherwise exit this window once done.')
    label6.pack()
    #Make sure TKinter window gets brought to front of screen
    final.lift()
